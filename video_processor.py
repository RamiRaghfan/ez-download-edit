...
def process_videos(videos_dict, originals_directory, clips_directory, final_serve_directory, global_merge_all, global_merged_title):
    all_clips = []  # Used for global merging if global_merge_all is true
    ...
    for video_file, video_data in videos_dict.items():
        ...
        for task_idx, task in enumerate(associated_tasks):
            for group in task['clip_groups']:
                ...
                # Extract all clips in the current group
                ...
                # Handle the merging logic for the current clipGroup
                if group['merge_required']:
                    # If merging is required, only output the merged clip, not individual ones
                    ...
                    if clips:
                        ...
                        # Move the merged clip to the final serve directory
                    ...
                    # Optionally add the merged clip for global merge
                    if global_merge_all:
                        all_clips.append(VideoFileClip(merged_filename))

                else:
                    # If no merging, move each clip to the final serve directory
                    for clip in clips:
                    ...
                    # Optionally add all individual clips for global merge
                    if global_merge_all:
                        all_clips.extend([VideoFileClip(clip) for clip in clips])

    # Global merge of all videos if global_merge_all is set to True
    if global_merge_all and all_clips:
        final_package_filename = os.path.join(final_serve_directory, f"{global_merged_title}.mp4")
        merged_package_clip = concatenate_videoclips(all_clips)
        merged_package_clip.write_videofile(final_package_filename, codec='libx264', audio_codec='aac')
